...
@section('content')
	<div class="card">
		...
		<div class="card-body">
			<form action="{{ route('admin.sanpham.them') }}" method="post" enctype="multipart/form-data" class="needs-validation" novalidate>
				@csrf
				...
				<div class="mb-3">
					...
					<input type="file" class="form-control @error('hinhanhkhac') is-invalid @enderror" id="hinhanhkhac" name="hinhanhkhac" value="{{ old('hinhanhkhac') }}" />
					@error('hinhanhkhac')
						<div class="invalid-feedback"><strong>{{ $message }}</strong></div>
					@enderror
				</div>
				...
				<div class="d-flex justify-content-between">
					...
					<a href="{{route('admin.sanpham')}}" class="btn btn-primary">Quay lại</a>
				</div>
			</form>
		</div>
	</div>
