			...
			<form action="{{ route('admin.sanpham.sua', ['id' => $sanpham->id]) }}" method="post" enctype="multipart/form-data">
				...
				<div class="mb-3">
					...
					@if(!empty($sanpham->hinhanh))
						<img class="d-block rounded img-thumbnail" src="{{ env('APP_URL') . '/storage/app/' . $sanpham->hinhanh }}" width="100" />
				...
				<div class="mb-3">
					<label class="form-label" for="hinhanhkhac">Hình ảnh khác</label>
					@if(!empty($sanpham->hinhanhkhac))
						<img class="d-block rounded img-thumbnail" src="{{ env('APP_URL') . '/storage/app/' . $sanpham->hinhanhkhac }}" width="100" />
						<span class="d-block small text-danger">Bỏ trống nếu muốn giữ nguyên ảnh cũ.</span>
					@endif
					<input type="file" class="form-control @error('hinhanhkhac') is-invalid @enderror" id="hinhanhkhac" name="hinhanhkhac" value="{{ $sanpham->hinhanhkhac }}" />
					@error('hinhanhkhac')
						<div class="invalid-feedback"><strong>{{ $message }}</strong></div>
					@enderror
				</div>
