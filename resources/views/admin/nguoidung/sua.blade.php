			...
			<form action="{{ route('admin.nguoidung.sua', ['id' => $nguoidung->id]) }}" method="post">
				...
				<div class="mb-3">
					...
					<select class="form-select @error('role') is-invalid @enderror" id="role" name="role" required>
						...
						<option value="nvtv" {{ ($nguoidung->role == 'nvtv') ? 'selected' : '' }}>Nhân viên tư vấn</option>
					</select>
					@error('role')
						<div class="invalid-feedback"><strong>{{ $message }}</strong></div>
					@enderror
				...
				<div class="mb-3 form-check">
					<input class="form-check-input" type="checkbox" id="change_password_checkbox"  name="change_password_checkbox" {{old('change_password_checkbox')? 'checked' : false}} />
